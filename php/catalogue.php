...
$page = min(max((int) ($_GET['page'] ?? 1), 1), $totalPages);
...
<head>
  ...
  <section id="results" class="showroom reveal reveal-up">
    ...
    $qs = http_build_query(array_filter([
        'q' => $search, 'brand' => $brand, 'price_min' => $price_min,
        'price_max' => $price_max, 'year' => $year, 'sort' => $sort,
    ]));
    $qs = $qs !== '' ? $qs . '&' : '';
    ?>
    <?php if ($totalPages > 1): ?>
      <nav class="pagination reveal reveal-up" aria-label="Pagination">
        <span class="pagination-info"><?= $total ?> véhicule<?= $total > 1 ? 's' : '' ?> — page <?= $page ?> / <?= $totalPages ?></span>
        <div class="pagination-links">
          <?php if ($page > 1): ?><a class="pagination-link" href="?<?= $qs ?>page=<?= $page - 1 ?>">‹ Précédent</a><?php endif; ?>
          <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a class="pagination-link <?= $i === $page ? 'active' : '' ?>" href="?<?= $qs ?>page=<?= $i ?>"><?= $i ?></a>
          <?php endfor; ?>
          <?php if ($page < $totalPages): ?><a class="pagination-link" href="?<?= $qs ?>page=<?= $page + 1 ?>">Suivant ›</a><?php endif; ?>
        </div>
      </nav>
    <?php endif; ?>
  </section>
