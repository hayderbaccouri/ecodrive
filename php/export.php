...
function sqlDump($conn, $dbname) {
    ...
    echo "-- EcoDrive — Backup du " . date('Y-m-d H:i:s') . "\n--\n\n";
...
switch ($type) {
    ...
    case 'reservations':
        ...
        $headers = ['ID', 'Client', 'Email', 'Marque', 'Modèle', 'Date essai', 'Début', 'Fin', 'Statut', 'Notes', 'Créé le'];
        $rows = [];
        foreach ($data as $r) {
    ...
    case 'voitures':
        ...
        $headers = ['ID', 'Marque', 'Modèle', 'Année', 'Prix', 'Description', 'Image', 'Page détails', 'Créé le'];
        $rows = [];
        foreach ($data as $r) {
    ...
    case 'bornes':
        ...
        $headers = ['ID', 'Nom', 'Modèle', 'Puissance', 'Prix', 'Description', 'Image', 'Page détails', 'Créé le'];
        $rows = [];
        foreach ($data as $r) {
    ...
    case 'newsletter':
        $data = $conn->query("SELECT id, email, subscribed_at FROM newsletter_subscribers ORDER BY subscribed_at DESC")->fetch_all(MYSQLI_ASSOC);
        $headers = ['ID', 'Email', "Date d'inscription"];
        $rows = [];
        foreach ($data as $s) {
            $rows[] = [$s['id'], $s['email'], $s['subscribed_at']];
        }
        csvOutput('newsletter.csv', $headers, $rows);
        break;

    case 'audit':
        $data = $conn->query("SELECT * FROM admin_audit ORDER BY created_at DESC")->fetch_all(MYSQLI_ASSOC);
        $headers = ['ID', 'Admin ID', 'Admin', 'Action', 'Détails', 'Date'];
        $rows = [];
        foreach ($data as $a) {
            $rows[] = [$a['id'], $a['admin_id'], $a['admin_name'], $a['action'], $a['details'] ?? '', $a['created_at']];
        }
        csvOutput('audit.csv', $headers, $rows);
        break;
