...
$stmt = $conn->prepare("SELECT nom, email, created_at FROM utilisateur WHERE id_utilisateur = ?");
...
$moisFrMembre = ['janvier','février','mars','avril','mai','juin','juillet','août','septembre','octobre','novembre','décembre'];
$membreDepuis = !empty($user['created_at']) ? $moisFrMembre[(int)date('n', strtotime($user['created_at'])) - 1] . ' ' . date('Y', strtotime($user['created_at'])) : '';
...
$stmt->close();
$reservationsActives = $counts['pending'] + $counts['confirmed'];
...
<?php $asset_base = '../'; include __DIR__ . '/partials/header.php'; ?>
  ...
  <main class="main-wrap page-fade-in">
    ...
    <div class="client-hero">
      ...
      <div class="client-hero-text">
        ...
        <div class="hero-meta">
          ...
          <span><strong><?= $reservationsActives ?></strong> essai(s) actif(s)</span>
        </div>
    ...
    <div class="client-section" style="margin-bottom:2rem">
      <div class="next-resa">
        <div>
          <div class="next-resa-label">🗓 Prochain essai</div>
          <div class="next-resa-title"><?= htmlspecialchars($nextReservation['marque'].' '.$nextReservation['modele']) ?> — <?= date('d/m/Y', strtotime($nextReservation['date_essai'])) ?></div>
          <div class="next-resa-time"><?= htmlspecialchars($nextReservation['heure_debut']) ?> → <?= htmlspecialchars($nextReservation['heure_fin']) ?></div>
        </div>
    ...
    <div class="client-section">
      ...
      new Chart(document.getElementById('chartStatus'), {
        ...
        data: {
          ...
          datasets: [{ data: [<?= (int)$counts['pending'] ?>, <?= (int)$counts['confirmed'] ?>, <?= (int)$counts['cancelled'] ?>], backgroundColor: ['#f59e0b', '#00e5a0', '#ef4444'], borderWidth: 0 }]
        },
      ...
      <?php else: ?>
        ...
        <table>
          ...
          <tr>
            <td data-label="Voiture" class="cell-strong"><?= htmlspecialchars($r['marque'].' '.$r['modele']) ?></td>
