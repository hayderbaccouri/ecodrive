...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $messageType = 'error';
    } elseif (rate_limit_check($conn, 'reset:' . strtolower($email), 3, 3600) ||
              rate_limit_check($conn, 'reset:' . rate_limit_ip(), 5, 3600)) {
        ...
        if ($user) {
            ...
            rate_limit_hit($conn, 'reset:' . strtolower($email));
            rate_limit_hit($conn, 'reset:' . rate_limit_ip());
        }
