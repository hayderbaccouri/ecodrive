...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $password === '') {
        ...
        $stmt = $conn->prepare('SELECT id_utilisateur, nom, email, mot_de_passe, role, email_verified FROM utilisateur WHERE email = ? LIMIT 1');
        ...
        if ($stmt) {
            ...
            if ($user && password_verify($password, $user['mot_de_passe'])) {
              if ((int)($user['email_verified'] ?? 1) !== 1) {
                // Compte non vérifié : refus + proposition de renvoi
  ...
  <div class="login-form-col">
    <div class="login-form-wrap">
      ...
      <?php if (isset($_GET['verify'])): ?>
        <div class="login-success">✓ Compte créé avec succès ! Un lien de vérification vient d'être envoyé à votre adresse e-mail.</div>
      <?php elseif (isset($_GET['registered'])): ?>
        <div class="login-success">✓ Compte créé avec succès. Vous pouvez maintenant vous connecter.</div>
      <?php endif; ?>

      <?php if (!empty($needVerify)): ?>
        <div class="login-error">⚠ Votre adresse e-mail n'a pas encore été vérifiée.
          <form method="post" action="verifier-email.php" style="margin-top:.5rem">
            <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
            <input type="hidden" name="email" value="<?= htmlspecialchars($verifyEmail ?? '') ?>">
            <button type="submit" style="background:none;border:none;color:inherit;text-decoration:underline;cursor:pointer;padding:0">Renvoyer le lien de vérification →</button>
          </form>
        </div>
      <?php elseif ($error): ?>
