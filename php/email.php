...
function emailVerification($email, $token) {
    $link = app_url('/php/verifier-email.php?token=' . urlencode($token));
    $body = "<h2>Vérification de votre adresse e-mail</h2>"
          . "<p>Bonjour,</p>"
          . "<p>Cliquez sur le bouton ci-dessous pour confirmer votre adresse e-mail et activer votre compte :</p>"
          . "<p><a href='$link' style='display:inline-block;padding:12px 24px;background:#0A7DA8;color:#fff;text-decoration:none;border-radius:999px;font-weight:400'>Vérifier mon e-mail</a></p>"
          . "<p>Ce lien expirera dans 24 heures.</p>"
          . "<p>Si vous n'avez pas créé de compte, ignorez cet email.</p>";
    return sendEmail($email, "Vérifiez votre e-mail — EcoDrive", $body);
}

function emailPasswordReset($email, $token) {
    $link = app_url('/php/reinitialiser-mot-de-passe.php?token=' . urlencode($token));
...
function emailWelcome($user) {
    $catalogueLink = app_url('/php/catalogue.php');
    $body = "<h2>Bienvenue chez EcoDrive !</h2>"
          ...
          . "<p><a href='{$catalogueLink}' style='display:inline-block;padding:12px 24px;background:#0A7DA8;color:#fff;text-decoration:none;border-radius:999px;font-weight:400'>Parcourir le catalogue</a></p>";
...
function emailAdminNewReservation($adminEmail, $user, $car, $reservation) {
    $adminLink = app_url('/php/admin.php');
    $body = "<h2>Nouvelle réservation</h2>"
          ...
          . "<p><a href='{$adminLink}'>Gérer dans l'admin</a></p>";
